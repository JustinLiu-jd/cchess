...
class Cannon(Chessman):
    ...
    def calc_moving_list(self):
        ...
        current_color = super(Cannon, self).is_red                  #添加吃子时的path
...
class Mandarin(Chessman):
    ...
    def calc_moving_list(self):
        ...
        super(Mandarin, self).add_from_probable_points(         #从可能的点中添加path
            probable_moving_points, current_color)
...
class Pawn(Chessman):
    ...
    def calc_moving_list(self):
        ...
        if current_h_c * self.__direction >= self.__river * self.__direction:       #过河
            probable_moving_points.append(
