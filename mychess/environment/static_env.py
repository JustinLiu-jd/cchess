...
from mychess.environment.light_env.common import *
from mychess.environment.lookup_tables import Winner, Fen_2_Idx, flip_move
...
def done(state, turns=-1, need_check=False):
    if 's' not in state:                # check red_k and black_k to judge isGameEnd
        return (True, 1, None)
    if 'S' not in state:
        return (True, -1, None)
    ...
    winner = None
    v = 0           # black wim: -1, red win: 1
    for i in range(BOARD_HEIGHT):           # find the locations of the two kings
    ...
    if red_k[0] == 0 and red_k[1] == 0:
        ...
        winner = Winner.red
        v = 1
    elif red_k[1] == black_k[1]:        # 对将，走棋方赢（红赢）
        has_block = False
        i = red_k[0] + 1
        while i < black_k[0]:
            if board[i][red_k[1]] != '.':
    ...
    check = False
    if winner is None:
        legal_moves = get_legal_moves(state, board)     # 生成合理走法
